...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '#t8p-mob-wm{height:28px}',
      '#t8p-mob-wm img{height:100%;width:auto}',
  ...
  function buildMobileHome() {
    ...
    Object.keys(DATA).filter(function(sl){
      ...
      if (vids.length > 0) {
        ...
        ifr.src = 'https://player.vimeo.com/video/'+vid+'?background=1&autoplay=1&loop=1&muted=1&autopause=0';
        ifr.setAttribute('frameborder','0');
        ifr.setAttribute('allow','autoplay');
        tile.appendChild(ifr);
      }

      /* placeholder img (shows behind iframe, acts as poster) */
      var ph = document.createElement('img');
      ph.src = ''; ph.alt = (d.t || sl).replace(/^"|"$/g,'');
      tile.appendChild(ph);
