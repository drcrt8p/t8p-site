...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '#t8p-panel-gallery-hdr{padding:24px 0 20px 0;max-width:1200px;margin:0 auto;box-sizing:border-box}',
      '@media(max-width:1440px){#t8p-panel-gallery-hdr{padding:24px 24px 20px 24px;max-width:none;margin:0}}',
      '@media(max-width:960px){#t8p-panel-gallery-hdr{padding:24px 24px 20px 24px}}',
      '@media(max-width:600px){#t8p-panel-gallery-hdr{padding:20px 20px 16px 20px}}',
      ...
      '@media(max-width:960px){.t8p-gal-grid{grid-template-columns:repeat(3,1fr);gap:10px}}',
      '@media(max-width:600px){.t8p-gal-grid{grid-template-columns:repeat(2,1fr);gap:8px;padding:0 20px}}',
      '.t8p-gal-tile{display:flex;flex-direction:column;cursor:pointer;transition:opacity .2s}',
      ...
      'z-index:10001}',
      '#t8p-lb-close{width:36px;height:36px;border-radius:50%;border:1px solid rgba(201,230,253,.25);',
      'background:transparent;color:#c9e6fd;font-size:16px;cursor:pointer;',
      'display:flex;align-items:center;justify-content:center;transition:border-color .2s}',
      '#t8p-lb-close:hover{border-color:#c9e6fd;background:rgba(201,230,253,.06)}',
  ...
  function buildDockPanel_woxerpolaroid(pp, vidsAll, d, title, desc, release, bodyText) {
    ...
    function lbOpen(idx){
      ...
      if (isFirst) {
        /* Opening from a tile — no fade on the first show */
        lbImg.classList.remove('fading');
        lbImg.src = newSrc;
        lb.classList.add('open');
      } else {
        /* Navigating between images — fade out, swap, fade in. */
        lbImg.classList.add('fading');
        setTimeout(function(){
          if (lb._idx !== newIdx) return; /* stale — user clicked again */
          lbImg.src = newSrc;
